<template>
    <div id="functionTest">
            ...
            <a-row>
                <template v-for="(value, index) in queryInfoList">
                    <a-col :span="6" :key="index" v-if="value.view !== 'popup'" :style="{ display: index < (expand ? queryInfoList.length : 2) ? 'block' : 'none', paddingRight: '12px' }">
                        ...
                        <a-form-item v-if="value.view == 'sel_search' || value.view == 'searchSelect'" :label="value.label" :key="index">
                            <a-select show-search option-filter-prop="children" :filter-option="filterOption" @focus="handleFocus(value.field)" @blur="handleBlur" @change="handleSelect" v-decorator="[value.field]" :placeholder="'请输入' + value.label">
                                <!--  -->
                                <a-select-option v-for="item in content" :key="item.value" :value="item.value">
                                    {{ item.text }}
                                </a-select-option>
                            </a-select>
                        </a-form-item>
                        ...
                        <a-form-item v-if="value.view == 'list' || value.view == 'select' || value.view == 'radio'" :label="value.label" :key="index">
                            <a-select v-decorator="[value.field]" :placeholder="'请输入' + value.label">
                                <a-select-option v-for="item in dictOptions[value.field]" :key="item.value" :value="item.value">
                                    {{ item.text }}
                                </a-select-option>
                            </a-select>
                        </a-form-item>
                ...
                <div class="content-modal">
                    <div class="left-modal">
                        ...
                        <a-row v-for="(modelForm, i) in modelForms" :key="i">
                            ...
                            <a-col :span="8">
                                <a-form-model-item>
                                    <a-input :placeholder="'请输入值'" v-if="modelForm.type == 'input' || modelForm.type == 'text' || modelForm.type == 'image' || modelForm.type == 'password' || modelForm.type == 'file' || modelForm.type == 'cat_tree' || modelForm.type == 'sel_tree' || modelForm.type == 'markdown' || modelForm.type == 'umeditor'" allowClear style="width: 240px" v-model="modelForm.val" @change="textChange" />
                                    <a-input-number :placeholder="'请输入数字'" v-if="modelForm.type == 'number'" allowClear style="width: 240px" v-model="modelForm.val" @change="numberChange" />
                                    <a-date-picker :placeholder="'请选择日期'" v-if="modelForm.type == 'date'" allowClear style="width: 240px" v-model="modelForm.val" @change="timeChange" />
                                    <a-date-picker show-time :placeholder="'请选择日期时间'" v-if="modelForm.type == 'datetime'" allowClear style="width: 240px" v-model="modelForm.val" @change="timeChange" />

                                    <a-time-picker :placeholder="'请选择时间'" v-if="modelForm.type == 'time'" allowClear style="width: 240px" v-model="modelForm.val" @change="timeChange" />
                                    ...
                                    <j-select-multi-user v-if="modelForm.type == 'sel_user'" :placeholder="'请选择用户'" style="width: 240px" v-model="modelForm.val" :read-only="true" />
        ...
        <!-- 表格 -->
        <a-table :row-selection="rowSelection" :columns="columns" :loading="templateLoading" :data-source="msg" bordered :pagination="pagination" @change="handleTableChange" @expand="handleExpand">
            ...
            <div :style="formStatus == 'show' ? 'cursor: not-allowed;' : ''">
                <a-form :form="addForm" :labelCol="{ span: 4 }" :wrapperCol="{ span: 20 }" :class="formStatus == 'show' ? 'form-disabled' : ''">
                    <template v-for="(value, key) in formProps">
                        ...
                        <!-- 时间文本框 -->
                        <a-form-item v-if="value.view == 'time'" :label="value.title" :key="key" :class="formStatus == 'show' ? 'form-disabled' : ''">
                            <a-time-picker
                                v-decorator="[
                                    key,
                                    {
                                ...
                                v-decorator="[
                                    key,
                                    {
                                        rules: [
                                            ...
                                            value.maxLength > 0 ? { max: sssssvalue.maxLength, message: '超过最大长度' } : ''
                                        ]
                                    }
                                ]"
                                :placeholder="'请选择' + value.title"
                        ...
                        <a-form-item v-if="value.view == 'markdown'" :label="value.title" :key="key" :class="formStatus == 'show' ? 'form-disabled' : ''">
                            ...
                            <!-- <mavon-editor
                                ...
                                "
                            /> -->
                            <a-textarea v-decorator="[key]" placeholder="请输入" :rows="4" v-model="markdown" />
...
export default {
    ...
    methods: {
        ...
        //输入值的改变
        textChange(text) {
            // console.log(this.modelForms[0].val,"text")
            // this.time = text.data
        ...
        handleTableChange(page,filters,sorter) {
            ...
            // console.log('sorter:',sorter.columnKey,sorter.order);
            ...
            console.log(this.columnkey,"排序")
            this.getDataByTree(this.code);
        },
        // 外部 重置 ：清空查询框的内容，不干涉高级查询的内容
        resetTableData() {
            Object.assign(this.pagination, {
                current: 1
            });
            ...
            this.queryForm.validateFields((err, values) => {
                ...
                // values = '';
                // this.queryCondition 中存储的是 查询框和高级查询框中的 查询参数：所以只需要 清除掉关于查询框的参数即可
                delete this.queryCondition.t1;
                delete this.queryCondition.t2;
                delete this.queryCondition.t3;

                delete this.queryCondition.t4_begin;
                delete this.queryCondition.t4_end;
                delete this.queryCondition.t5_begin;
                delete this.queryCondition.t5_end;
                delete this.queryCondition.t6;
                delete this.queryCondition.t7;
                console.log(this.queryCondition, 'kong');
                // 请求当前页的数据
                this.getDataByTree(this.code);
            });
        },
        ...
        handleQuerySubmit(e) {
            ...
            this.queryForm.validateFields((err, values) => {
                ...
                console.log(values, '搜索values');
                // console.log(this.modelForms, '普通查询');
                this.queryCondition = values;
                ...
                // 高级查询时 对时间控件的输入值进行 时间格式 的更改
                if (form[0].type === 'date' || form[0].type === 'datetime') {
                    form[0].val = this.time;
                }
                if (form[0].type === 'time') {
                    form[0].val = this.time;
                }
                if(this.modelForms[0].val){
                    this.queryCondition.superQueryParams = encodeURI(JSON.stringify(form));
                }
                
                console.log(this.queryCondition, '普通搜索');
        ...
        loadFormItem(id) {
            ...
            getFormItem({ code: id })
                .then((res) => {
                    ...
                    // 省市联动
                    // console.log(this.formProps,"this.form省市")
                    if (this.formProps.province) {
                        // console.log("进入了哦")
                        this.table = this.formProps.province.config.table;
                        this.txt = this.formProps.province.config.txt;
                        this.pidField = this.formProps.province.config.pidField;
                        this.idField = this.formProps.province.config.idField;
                        this.key = this.formProps.province.config.key;
                        this.condition = this.formProps.province.config.condition;
                    }
        ...
        showModal() {
            ...
            if(this.table){   //只有 是link_down下拉联动组件模型时
                const table = this.table;
                const txt = this.txt;
                const pidField = this.pidField;
                const idField = this.idField;
            ...
            this.addForm.validateFields((err, values) => {
                for (let key in values) {
                    ...
                    if (this.formProps[key]?.view == 'date') {
                        ...
                        // console.log(this.markdown,"mark")
                        // console.log(values[key],"values[key]")
                        // console.log(this.markdownValues[key],"this.markdownValues[key]")
                        // // values[key] = this.markdownValues[key];
                        // values[key] = this.markdown
        ...
        getData(id, page) {
            ...
            // 升序降序 需要更改两个参数：column ，order
            params.column = 'createTime';
            if(this.columnkey){
                params.column = this.columnkey
            }
            params.order = 'desc';
            ...
            getTableData(id, params)
                .then((res) => {
                    // 高级查询成功 模态框消失
                    (this.visible1 = false), (this.dataResult = res.result);
                    // console.log(res.result,"获取表格de数据")
        ...
        loadTreeData(type) {
            ...
            for (const key in this.selectTreeList) {
                // console.log('selectTreeList',this.selectTreeList);
                let data = {};
            ...
            return new Promise((resolve) => {
                ...
                setTimeout(() => {
                    loadTreeDatas({
                        ...
                        .then((res) => {
                            console.log(this.selectTreeList,"this.selec")
                            let selectTreeList = this.selectTreeList;
                            let arr = [];
                            console.log(res.result,"自定义控件树")
                            res.result.forEach((item) => {
                                item.isLeaf = item.leaf;
                                item.pId = item.parentId;
                                item.id = item.key;
                            ...
                            // console.log(arr,"arr")
                            // console.log(selectTreeList[pid],"selectTreeList[pid]")
                            selectTreeList[pid] = selectTreeList[pid].concat([...arr]);
                            // console.log(selectTreeList[pid],"selectTreeList是")  
                            // console.log(selectTreeList,"selectTreeList")
                            this.selectTreeList = selectTreeList;
                        })
                        .catch((err) => {
                            this.$message.error(err);
        ...
        handleSearch(e) {
            ...
            // 获取时间控件 选择框的值
            // console.log("modelForms:",this.modelForms[0].val._i)
            // 获取输入框中的值
            // console.log('输入框的值', this.modelForms[0].val);
            ...
            console.log(this.modelForms, '高级查询');
            let form = JSON.parse(JSON.stringify(this.modelForms));
            form.forEach((item) => {
                delete item.enum;
                delete item.key;
            ...
            console.log(JSON.stringify(form), 'gaojie');
            ...
            // 高级查询bug：清除外部输入框的内容时，同时也将 高级查询输入框的内容清除
        },
        // 清空查询条件
        emptySearch() {
            this.queryCondition.superQueryParams = '';
