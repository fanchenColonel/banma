<template>
    <a-modal centered :title="name + '选择'" :width="width" :visible="visible" switchFullscreen @ok="handleOk" @cancel="close" cancelText="关闭">
        <a-row :gutter="18">
            <a-col :span="16">
                <!-- 查询区域 -->
                <a-form layout="inline" class="j-inline-form">
                    ...
                    <!-- 动态生成的查询条件 -->
                    <a-form-item label="性别：" v-show="showMoreQueryItems">
                        <a-select :queryConfig="queryConfig" style="width: 180px" @change="handleChange" placeholder="请选择性别">
                            <a-select-option value="1"> 男 </a-select-option>
                            <a-select-option value="2"> 女 </a-select-option>
                        </a-select>
                    </a-form-item>

                    <a-form-item label="生日：" v-show="showMoreQueryItems">
                        <a-date-picker placeholder="请选择出生日期" :queryConfig="queryConfig" style="width: 180px" />
                    </a-form-item>

                    <!-- <j-select-biz-query-item v-if="queryConfig.length > 0" v-show="showMoreQueryItems" :queryConfig="queryConfig" @pressEnter="searchQuery" /> -->
                ...
                <a-table size="middle" bordered :rowKey="rowKey" :columns="innerColumns" :data-source="dataSource" :pagination="ipagination" :scroll="{ y: 240 }" :rowSelection="{ selectedRowKeys, onChange: onSelectChange, type: multiple ? 'checkbox' : 'radio' }" :customRow="customRowFn" @change="handleTableChange"> </a-table>
            </a-col>
            <!-- 右边--已选用户 -->
            <a-col :span="8">
...
<script>
import { getAction } from '@/api/manage'; //ok
import Ellipsis from '@/components/Ellipsis'; //ok
// import { JeecgListMixin } from '@/mixins/JeecgListMixin'; //不要
import { cloneObject, pushIfNotExist } from '@/utils/util'; // OK
import JSelectBizQueryItem from './JSelectBizQueryItem';
import { getUserList } from '@/api/onlineFormFuncTest';
...
export default {
    ...
    methods: {
        ...
        queryOptionsByValue(value) {
            ...
            if (!notExist) return;
            console.log(this.valueUrl, this.listUrl);
