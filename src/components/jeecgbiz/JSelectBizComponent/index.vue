<template>
    <a-row class="j-select-biz-component-box" type="flex" :gutter="8">
        <a-col class="full" :class="{ full: !buttons }">
            <slot name="left">
                <a-select mode="multiple" :placeholder="placeholder" v-model="selectValue" :options="selectOptions" allowClear :disabled="disabled" :open="selectOpen" style="width: 100%" @dropdownVisibleChange="handleDropdownVisibleChange" @click.native="visible = true" />
            </slot>
        </a-col>
...
export default {
    ...
    components: { 'j-select-biz-component-modal': JSelectBizComponentModal },
    props: {
        value: {
            type: String,
            default: ''
